...
| `RATE_LIMIT_REDIS_URL`               | **Yes in production**  | omitted in non-production if unset                       | `rateLimitRedisUrl` — **`redis://`** / **`rediss://`** for shared HTTP rate limits. **Required when `NODE_ENV=production`** (startup error if missing). Optional in development (in-memory store). See [security-and-http-hardening.md](./security-and-http-hardening.md). |
