...
| 4    | **`createHttpRateLimiter`** output    | Rate limiting: **Redis** when **`RATE_LIMIT_REDIS_URL`** is set (**required** in **`NODE_ENV=production`**); in-memory only when unset in non-production |
