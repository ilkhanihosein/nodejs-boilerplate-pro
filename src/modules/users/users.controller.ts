...
import type { UpdateRoleBody, UserIdParams } from "./users.schemas.js";
...
export const getUserByIdHandler: RequestHandler = async (req, res, next) => {
  try {
    const params = requireValidatedParams(req) as UserIdParams;
...
export const updateUserRoleHandler: RequestHandler = async (req, res, next) => {
  try {
    const params = requireValidatedParams(req) as UserIdParams;
    const body = requireValidatedBody(req) as UpdateRoleBody;
